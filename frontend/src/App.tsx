...
const TRANSLATIONS: any = {
  FR: {
    ...
    duration: "DURÉE",
    delta: "DELTA",
    layer_clouds: "Couche Nuages (Sat)",
    layer_pillars: "Piliers de Couplage",
    responses: {
      ...
      europe: "L'analyse transcontinentale (UE) identifie une convergence de {val}%. Le couplage n'est plus un signal localisé, mais une nappe de résonance synchrone à l'échelle du continent, formant une structure géosystémique parfaitement cohérente.",
      how: "L'identification algorithmique ne cherche pas une causalité linéaire, mais une corrélation statistique entre une impulsion émotionnelle collective (ε) et une anomalie atmosphérique récurrente (z). Si le motif se répète avec un décalage L, le Kernel compile cette coïncidence comme une convergence géosystémique.",
      ...
      elaborate: "Imaginez que l'atmosphère de l'Europe soit la surface d'un tambour géant. Chaque choc collectif est un coup de baguette. À {val}%, le tambour tout entier se met à vibrer sur une seule note stable : c'est la 'nappe de résonance'. Ce n'est plus du simple bruit, mais une symphonie où l'émotion humaine donne le tempo aux vents et aux températures.",
  ...
  EN: {
    ...
    duration: "DURATION",
    delta: "DELTA",
    layer_clouds: "Cloud Layer (Sat)",
    layer_pillars: "Coupling Pillars",
    responses: {
      ...
      europe: "Transcontinental analysis (EU) identifies a {val}% convergence. Coupling is no longer a localized signal, but a synchronous resonance layer across the entire continent, forming a perfectly coherent geosystemic structure.",
      how: "Algorithmic identification does not seek direct linear causality, but a statistical correlation between collective emotional impulse (ε) and recurring atmospheric anomalies (z). If the pattern repeats with lag L, the Kernel compiles this coincidence as a geosystemic convergence.",
      ...
      elaborate: "Imagine Europe's atmosphere as the skin of a giant drum. Every collective shock is a drumstick hit. At {val}%, the entire drum starts vibrating to a single stable note: that's the 'resonance layer'. It's no longer just noise, but a symphony where human emotion sets the tempo for winds and temperatures.",
...
import { Thermometer, Users, Zap, Settings, Activity, Download, Info, HelpCircle, X, Globe, Clock, Send, Layers, CloudLightning } from 'lucide-react';
...
const EVENTS_DB: Record<string, Array<{ date: string, label: string, type: 'psy' | 'atm' }>> = {
  'FR': [
    { date: '1973-10-17', label: "Premier Choc Pétrolier - Crise Mondiale", type: 'psy' },
    { date: '1976-07-20', label: "Canicule historique - Été de la soif", type: 'atm' },
    { date: '1981-05-10', label: "Élection Mitterrand - Choc Social", type: 'psy' },
    { date: '1995-12-15', label: "Grèves records (Plan Juppé)", type: 'psy' },
    { date: '1999-12-26', label: "Tempête Lothar - Ravages climatiques", type: 'atm' },
    { date: '2003-08-10', label: "Canicule extrême (+15k décès)", type: 'atm' },
    { date: '2015-01-07', label: "Attentat Charlie Hebdo - Choc National", type: 'psy' },
    { date: '2015-11-13', label: "Attentats Paris - État de siège", type: 'psy' },
    { date: '2019-04-15', label: "Incendie Notre-Dame - Émoi mondial", type: 'psy' },
    { date: '2020-03-17', label: "Premier Confinement - Choc Viral", type: 'psy' }
  ],
  'DE': [
    { date: '1973-11-25', label: "Interdiction de circuler (Choc Pétrolier)", type: 'psy' },
    { date: '1977-10-18', label: "Automne Allemand (RAF) - Crise d'État", type: 'psy' },
    { date: '1989-11-09', label: "Chute du Mur de Berlin", type: 'psy' },
    { date: '1990-10-03', label: "Réunification Allemande", type: 'psy' },
    { date: '2013-06-01', label: "Inondations millénaires de l'Elbe", type: 'atm' },
    { date: '2015-01-20', label: "Sommet de l'Énergie à Berlin", type: 'psy' },
    { date: '2021-07-15', label: "Inondations meurtrières (Ahrweiler)", type: 'atm' }
  ],
  'UK': [
    { date: '1979-05-04', label: "Élection Margaret Thatcher", type: 'psy' },
    { date: '1987-10-16', label: "The Great Storm - Tempête record", type: 'atm' },
    { date: '1997-08-31', label: "Décès Princesse Diana - Deuil record", type: 'psy' },
    { date: '2005-07-07', label: "Attentats de Londres", type: 'psy' },
    { date: '2016-06-23', label: "Référendum Brexit - Choc Politique", type: 'psy' }
  ],
  'IT': [
    { date: '1978-05-09', label: "Assassinat Aldo Moro - Choc National", type: 'psy' },
    { date: '1980-08-02', label: "Attentat de Bologne", type: 'psy' },
    { date: '2009-04-06', label: "Séisme de L'Aquila", type: 'atm' },
    { date: '2015-02-20', label: "Carnaval de Venise - Affluence record", type: 'psy' },
    { date: '2019-11-12', label: "Acqua Alta historique - Venise sous l'eau", type: 'atm' }
  ],
  'ES': [
    { date: '1975-11-20', label: "Fin de la dictature de Franco", type: 'psy' },
    { date: '1981-02-23', label: "Tentative de Coup d'État (F23)", type: 'psy' },
    { date: '2004-03-11', label: "Attentats de Madrid (Atocha)", type: 'psy' },
    { date: '2017-08-17', label: "Attentats de Barcelone", type: 'psy' },
    { date: '2021-01-08', label: "Tempête Filomena - Madrid sous la neige", type: 'atm' }
  ],
  'PL': [
    { date: '1980-08-31', label: "Accords de Gdansk (Solidarnosc)", type: 'psy' },
    { date: '1997-07-10', label: "Inondations de l'Oder - Catastrophe polonaise", type: 'atm' },
    { date: '2010-04-10', label: "Crash de Smolensk - Choc National", type: 'psy' }
  ]
};
...
const App = () => {
  ...
  const activeEvents = useMemo(() => {
    ...
    // 1. Ajouter les événements statiques du pays ou de l'EU
    if (selectedCountry === 'EU') {
      Object.values(EVENTS_DB).forEach(list => {
        combinedEvents = [...combinedEvents, ...list];
      });
    } else {
      combinedEvents = EVENTS_DB[selectedCountry] || [];
    }

    // 2. Ajouter les événements issus de historical_analysis.json (Automation)
    if (histData?.timeline_shocks) {
      const histShocks = histData.timeline_shocks.map((s: any) => ({
        date: s.date_start,
        label: s.label,
        type: s.type === 'atm' ? 'atm' : 'psy',
        desc: s.description,
        isHist: true
      }));
      combinedEvents = [...combinedEvents, ...histShocks];
    }

    // 3. Filtrage par date (fenêtre de 3 jours)
    const filtered = combinedEvents.filter((e: any) => {
    ...
    // 4. Déduplication par label
    return filtered.filter((v, i, a) => a.findIndex(t => (t.label === v.label)) === i);
  }, [selectedCountry, currentDate, histData]);
  ...
  // Synchronisation du chat avec la sélection
  useEffect(() => {
    const newInitial = t('initial_msg').replace('{decade}', activeDecade).replace('{country}', selectedCountry);
    
    if (messages.length <= 1) {
      // Si on est au début, on met à jour le message d'accueil
      setMessages([{ role: 'bot', text: newInitial }]);
    } else {
      // Si la conversation est engagée, on notifie le changement de canal
      const lastMsg = messages[messages.length - 1];
      if (lastMsg.role === 'bot' && lastMsg.text.startsWith(lang === 'FR' ? "Passage au canal" : "Switching to channel")) {
        // Éviter les doublons si on clique plusieurs fois
        return;
      }
      setMessages(prev => [...prev, { 
        role: 'bot', 
        text: lang === 'FR' ? `Passage au canal ${selectedCountry} opérationnel.` : `Switching to channel ${selectedCountry}.` 
      }]);
    }
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      const countriesSnapshot: Record<string, any> = {};
      if (computed.countries) {
        for (const [code, data] of Object.entries(computed.countries) as [string, any][]) {
          countriesSnapshot[code] = {
            ci_series: data.ci_series ? [data.ci_series[currentTimeIndex]] : [0],
            psy_res: data.psy_res ? [data.psy_res[currentTimeIndex]] : [0],
            atm: data.atm ? [data.atm[currentTimeIndex]] : [0],
            // Include last 10 CI values for trend
            ci_trend: data.ci_series ? data.ci_series.slice(Math.max(0, currentTimeIndex - 9), currentTimeIndex + 1) : [],
          };
        }
      }
      ...
      const context = {
        ...
        currentTimeIndex,
        params,
        countriesData: countriesSnapshot,
      ...
      if (data.response) {
        ...
        setMessages(prev => [...prev, { role: 'bot', text: `⚠️ ERREUR KERNEL : ${data.error}` }]);
      }
    } catch (err: any) {
      // Fallback to basic response if API is unreachable
      const globalVal = (computed.ci_global_series?.[currentTimeIndex] || 0) * 100;
      setMessages(prev => [...prev, { 
        role: 'bot', 
        text: `⚠️ CONNEXION LLM INDISPONIBLE.\n\nMode dégradé : CI global = ${globalVal.toFixed(2)}% | Canal ${selectedCountry} | ${activeDecade}\n\nVérifiez que le serveur API Kernel est lancé (npm run dev dans backend/api/).`
      }]);
    } finally {
      setIsThinking(false);
    }
  ...
  return (
    <>
      ...
      <aside className="sidebar">
        ...
        <section>
          <h3><Layers size={14} /> {lang === 'FR' ? 'AFFICHAGE' : 'VIEW LAYERS'}</h3>
          <div className="card flex flex-col gap-3">
            <div className="flex justify-between items-center">
              <span className="small uppercase mono text-dim">{t('layer_clouds')}</span>
              <input type="checkbox" checked={showClouds} onChange={() => setShowClouds(!showClouds)} 
                style={{ cursor: 'pointer', accentColor: 'var(--text-accent)' }} />
            </div>
            <div className="flex justify-between items-center">
              <span className="small uppercase mono text-dim">{t('layer_pillars')}</span>
              <input type="checkbox" checked={showPillars} onChange={() => setShowPillars(!showPillars)} 
                style={{ cursor: 'pointer', accentColor: 'var(--text-accent)' }} />
            </div>
            <div className="flex justify-between items-center">
              <span className="small uppercase mono text-dim">{lang === 'FR' ? 'Monitoring' : 'Monitoring'}</span>
              <input type="checkbox" checked={showLabels} onChange={() => setShowLabels(!showLabels)} 
                style={{ cursor: 'pointer', accentColor: 'var(--text-accent)' }} />
            </div>
            <div className="flex justify-between items-center">
              <span className="small uppercase mono text-dim">{lang === 'FR' ? 'Auto-Rotation' : 'Auto-Spin'}</span>
        ...
        <section>
          ...
          <div className="flex flex-col gap-2">
            {activeEvents.length > 0 ? activeEvents.map((e: any, i: number) => (
              <div key={i} className="card" style={{ padding: '12px', borderLeft: `3px solid ${e.type === 'psy' ? '#60a5fa' : '#fb923c'}`, background: 'rgba(255,255,255,0.02)' }}>
                <div className="flex justify-between items-start mb-1">
                  <div className="mono" style={{ fontSize: '0.65rem', opacity: 0.6 }}>{e.date} // {e.type.toUpperCase()}</div>
                  <a 
                    href={`https://fr.wikipedia.org/wiki/Special:Search?search=${encodeURIComponent(e.label)}`} 
                    target="_blank" 
                    rel="noopener noreferrer" 
                    className="text-accent hover:underline mono bold"
                    style={{ fontSize: '0.6rem' }}
                  >
                    [WIKIPEDIA]
                  </a>
                </div>
                <div className="mono small uppercase mt-1 bold text-accent" style={{ letterSpacing: '1px' }}>{e.label}</div>
                
                {/* BULLETIN MÉTÉO (Replacing clouds visualization with precise text) */}
                <div style={{ marginTop: '10px', padding: '8px', background: 'rgba(56, 189, 248, 0.05)', border: '1px solid rgba(56, 189, 248, 0.2)', borderRadius: '2px' }}>
                  <div className="mono text-accent bold uppercase" style={{ fontSize: '0.45rem', marginBottom: '4px', letterSpacing: '1px' }}>
                    <CloudLightning size={10} style={{ verticalAlign: 'middle', marginRight: '4px' }} />
                    BULLETIN MÉTÉO KERNEL
                  </div>
                  <div className="mono text-dim" style={{ fontSize: '0.65rem' }}>
                    {e.climate_desc || (lang === 'FR' ? "Analyse atmosphérique en cours..." : "Atmospheric analysis pending...")}
                  </div>
                </div>

                {e.desc && (
                  <div className="mono text-dim mt-2 pt-2" style={{ fontSize: '0.65rem', borderTop: '1px solid rgba(255,255,255,0.05)', fontStyle: 'italic' }}>
                    {e.desc}
                  </div>
                )}
              </div>
        ...
        <section>
          <h3><Globe size={14} /> {lang === 'FR' ? 'CORRÉLATIONS : 50 ANS D\'HISTOIRE' : 'HISTORICAL CORRELATIONS'}</h3>
          <div className="custom-scrollbar" style={{ maxHeight: '450px', overflowY: 'auto', paddingRight: '10px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            
            {histData?.patterns.map((p: any, i: number) => {
              const allMatches = p.matching_events || [];
              
              // Map label to simpler narrative
              let title = p.label;
              if (p.label.includes('THERMIQUE')) title = lang === 'FR' ? 'CHOC >> CHALEUR' : 'SHOCK >> HEATWAVE';
              if (p.label.includes('BLOCAGE')) title = lang === 'FR' ? 'CHOC >> STABILITÉ' : 'SHOCK >> STABILITY';
              if (p.label.includes('AÉROSOLS')) title = lang === 'FR' ? 'CHOC >> LUMINOSITÉ' : 'SHOCK >> BRIGHTNESS';

              let frequency = lang === 'FR' ? 'MODÉRÉ' : 'MODERATE';
              if (p.recurrence > 50) frequency = lang === 'FR' ? 'SOUVENT' : 'FREQUENT';
              if (p.recurrence > 70) frequency = lang === 'FR' ? 'SYSTÉMIQUE' : 'SYSTEMIC';

              return (
                <div key={i} className="animate-fade-in">
                  <div className="flex justify-between items-baseline mb-3">
                    <span className="mono bold text-accent" style={{ fontSize: '0.85rem', letterSpacing: '1px' }}>{title}</span>
                    <span className="mono p-1" style={{ fontSize: '0.5rem', background: 'var(--text-accent)', color: '#000', fontWeight: 900, borderRadius: '2px' }}>
                      {frequency}
                    </span>
                  </div>
                  ...
                  {allMatches.length > 0 ? (
                    <div className="flex flex-col gap-4">
                       {allMatches.map((me: any, idx: number) => (
                         <div key={idx} style={{ paddingLeft: '14px', borderLeft: '1px solid rgba(255,255,255,0.1)' }}>
                            <div className="mono bold mb-1" style={{ fontSize: '0.9rem', color: '#fff' }}>{me.date}</div>
                            <div className="mono uppercase text-accent mb-2" style={{ fontSize: '0.65rem', fontWeight: 800 }}>{me.label}</div>
                            
                            <div className="mono text-dim leading-relaxed italic" style={{ fontSize: '0.65rem', marginBottom: '10px', background: 'rgba(255,255,255,0.02)', padding: '6px' }}>
                               "{me.event_desc}"
                            </div>

                            <div className="flex flex-wrap gap-4 mono" style={{ fontSize: '0.55rem' }}>
                               <div className="flex items-center gap-1">
                                 <span className="text-dim">ZONE:</span> <span className="text-white">{me.country}</span>
                               </div>
                               <div className="flex items-center gap-1">
                                 <span className="text-dim">IMPACT KERNEL:</span> <span style={{ color: '#fbbf24', fontWeight: 800 }}>{me.delta}</span>
                               </div>
                               <div className="flex items-center gap-1">
                                 <span className="text-dim">DURÉE:</span> <span className="text-white">{me.duration}</span>
                               </div>
                            </div>
                         </div>
                       ))}
                    ...
                    <div className="mono text-dim italic opacity-50 ml-4" style={{ fontSize: '0.6rem' }}>
                      &gt; {lang === 'FR' ? 'Aucun point témoin sur cette décennie.' : 'No witnesses found for this decade.'}
                    </div>
                  )}
                </div>
              );
      ...
      <main className="map-container">
        ...
        <EuropeMap 
          ...
          hideLabels={!showLabels || showProjectInfo || showKernelDialog}
          showPillars={showPillars}
          autoRotate={autoRotate}
          atmValue={currentCountry?.atm?.[currentTimeIndex] || 0}
          eventType={activeEvents[0]?.type}
        />
