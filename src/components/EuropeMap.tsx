...
const CountryPulse = ({ position, color, label, ci, hideLabels }: CountryMarkerProps) => {
  ...
  useFrame(({ clock }) => {
    if (beamRef.current) {
      ...
      beamRef.current.scale.y = Math.max(0.01, ci * 10 * s);
      (beamRef.current.material as THREE.MeshBasicMaterial).opacity = 0.1 + ci * 2;
    }
  });
  ...
  return (
    <group position={position}>
      ...
      {/* Coupling Beam (Observational Pillar) */}
      <mesh ref={beamRef} rotation={[Math.PI / 2, 0, 0]} position={[0, 0, 0.05]}>
        <cylinderGeometry args={[0.01, 0.04, 1, 12]} />
        <meshBasicMaterial color="#22c55e" transparent opacity={0} />
      </mesh>

      <mesh rotation={[-Math.PI / 2, 0, 0]}>
        <ringGeometry args={[0.03, 0.03 + ci * 0.5, 32]} />
...
const Earth = ({ selectedCountry, onSelectCountry, data, indicator, timeIndex, hideLabels }: GlobeProps) => {
        ...
        if (!isSelected) {
          return (
            <group key={c.code} position={c.pos}>
              ...
              {/* Pillar even when not selected if CI > 0 */}
              <mesh rotation={[Math.PI / 2, 0, 0]} position={[0, 0, 0.02]}>
                <cylinderGeometry args={[0.005, 0.01, c.ciVal * 5, 8]} />
                <meshBasicMaterial color="#22c55e" transparent opacity={c.ciVal * 2} />
              </mesh>
            </group>
          );
        }
